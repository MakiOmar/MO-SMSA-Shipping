...
<body>
	...
	<div class="container" style="margin-top:2%">
			<a href="<?php echo admin_url( 'edit.php?post_type=shop_order' );?>" class="smsa_action">Back to Orders</a>
   <p><center></center></p>
...
if($_POST)
{
					...
					$sett = get_option('woocommerce_mo-shipping-integration_settings');
                ...
                if ($token)
                {
					...
					foreach($_POST as $post)
					{
						...
						if($post['parcels']>0 && $post['parcels']<4 && $post['weight']>0)
						{
							...
							$resp1 = $api->create_shipment($shipping_data);
							// echo '<pre>';
							// print_r($re);
							// echo '</pre>';
							...
							if (isset($resp1['sawb']))
							{
							 ...
							 update_post_meta($post['order_id'], 'mo_shipping_awb_no', $resp1['sawb']);
							 update_post_meta($post['order_id'], 'mo_shipping_awb_no_link', 'https://www.smsaexpress.com/trackingdetails?tracknumbers='.$resp1['sawb']);
                             ...
                             $order = new WC_Order($post['order_id']);
								$order->update_status('printing-smsa', __( 'تم إنشاء بوليصة الشحن سمسا', 'flatsome' ));
							}
...
else
{
		...
		{
			$awb_nn = get_post_meta($order_id, 'mo_shipping_awb_no',true);
           if($awb_nn=="")
           {
  ...
  <input type="hidden" name="<?php echo $i;?>[action]" value="create_shipment">
    ...
    <div class="row">
      <div class="col">
        <label>City</label>
		<?php
			$customer_city = 'No city details';
			if ( ! empty( $order_data['billing']['city'] ) ) {
				$customer_city = $order_data['billing']['city'];
			} elseif ( ! empty( $order_data['shipping']['city'] ) ) {
				$customer_city = $order_data['shipping']['city'];
			} elseif ( ! empty( $order_data['billing']['state'] ) ) {
				$customer_city = $order_data['billing']['state'];
			}
		?>
        <input type="text" class="form-control"  name="<?php echo $i;?>[addressCity]" value="<?php echo $customer_city ?>" required>
      </div>
      <div class="col">
